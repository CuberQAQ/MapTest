...
class Session extends EventBus {
  ...
  checkIfReceiveAllChunks() {
    ...
    for (let i = 1; i <= this.count; i++) {
      ...
      this.tempBuf = this.tempBuf ? Buffer.concat([this.tempBuf, buf]) : buf; // chunks 拼接操作
    }
...
export class MessageBuilder extends EventBus {
  constructor({ appId = 0, appDevicePort = 20, appSidePort = 0, ble }) {
    super();
  ...
  onFragmentData(bin) {
    ...
    if (data.flag === MessageFlag.App && data.type === MessageType.Shake) {
      ...
      DEBUG && logger.debug("appSidePort=>", data.port2);
